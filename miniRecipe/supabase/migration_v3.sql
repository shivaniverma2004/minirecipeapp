-- v3: UUID text case + avatar storage RLS (Swift vs Postgres auth.uid() casing).
-- Run in Supabase SQL Editor on existing projects. Same as fix_rls_case.sql.
...
-- Avatar object key must match user id (case-insensitive)
...
-- Profile row if signup trigger missed
